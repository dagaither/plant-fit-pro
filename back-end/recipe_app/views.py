...
class AllFavorites(TokenReq):
    def get(self, request):
        user_recipe = Recipe.objects.get(app_user=request.user)
...
class AddToFavorites(TokenReq):
    def get(self, request, id):
        ...
        response = requests.get(url, headers=headers)
        json_response = response.json()
        return self.save_favorite(request.user, json_response)

    def save_favorite(self, user, dish):
        recipe, _ = Recipe.objects.get_or_create(app_user=user)
